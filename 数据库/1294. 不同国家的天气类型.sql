...
select a.country_name , case when avg(b.weather_state) <= 15 then 'Cold' when avg(b.weather_state) >= 25 then 'Hot' else 'Warm' end as weather_type from Countries a join Weather b using(country_id) where b.day like '2019-11%' group by a.country_name